...
KVTError kvt_scan(uint64_t tx_id, uint64_t table_id, const std::string& key_start, 
    ...
    results.clear(); // Make sure results is cleared before use
    KVTError result = g_kvt_manager->scan(tx_id, table_id, key_start, key_end, num_item_limit, results, error_msg);
    if (result != KVTError::SUCCESS) {
        DEBUG(std::cout << " -> ERROR: " << error_msg << std::endl);
    } else {
        DEBUG(std::cout << " -> SUCCESS, count=" << results.size());
        // Additional debug to verify data is actually there
        if (results.size() > 0) {
            std::cout << " (first key len=" << results[0].first.length() << ", val len=" << results[0].second.length() << ")";
        }
...
KVTError KVTMemManager2PL::scan(uint64_t tx_id, uint64_t table_id, const std::string& key_start,
    ...
    std::cout << "DEBUG scan: temp_results.size()=" << temp_results.size() << ", num_item_limit=" << num_item_limit << std::endl;
    for (const auto& [key, value] : temp_results) {
        std::cout << "  DEBUG adding to results: key_len=" << key.length() << ", val_len=" << value.length() << std::endl;
        results.emplace_back(key, value);
        if (results.size() >= num_item_limit) {
            break;
        }
    }
    std::cout << "DEBUG scan: final results.size()=" << results.size() << std::endl;
        ...
        std::vector<std::pair<std::string, std::string>>& results, std::string& error_msg) 
{
    ...
    std::cout << "DEBUG OCC scan: results_writes.size()=" << results_writes.size() 
              << ", results_table.size()=" << results_table.size() << std::endl;
    results_table.insert(results_writes.begin(), results_writes.end());
    std::cout << "DEBUG OCC scan: merged results_table.size()=" << results_table.size() << std::endl;
    ...
    std::cout << "DEBUG OCC scan: final results.size()=" << results.size() << std::endl;
    return KVTError::SUCCESS;
}
